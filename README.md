...
在数据方面，static中的car_data_15features.xlsx是我用原始数据（car_data.xlsx）在XGBoost的特征重要性筛选前15个最具影响力的特征而得到的，我的写入脚本在car\management\commands里，里面包含填补缺失值、写入数据、更新数据、训练模型的脚本，如果你使用你的数据，在写入和更新里面找到pd.read_excel就是我文件的写入目录，你可以自行更改，但是尽量保证字段和特征与我的示例数据一致，否则需要你自己修改写入的字段与类型。

训练模型脚本模组也是固定参数，你可以自行更改XBGoost的学习率和早停条件。
# 未来开发路线
未来我将以方便协作开发方向设计，着重开发系统的高适应性，可以实现动态数据集写入，包括在线动态爬虫（这部分可能需要较长时间，因为涉及一些汽车公司网站的数据许可）还有离线数据写入（为了防止数据库的数据泄露与污染，我将设计开发者角色来登录我的系统并在我的管理员下审核才能写入），其次我会优化预测模型，将原有的单一XGBoost模型与MLP模型结合成大模型，且类似于Chat GPT的角色定位，将模型更注重汽车价格预测，还有后续的时序预测，这是一项艰巨的工作，我大学四年学习的内容尚浅，希望社区大佬支持。
# Additional Information
The root directory of my working directory here is named NEV_Price_System, and you can rename it to something else (but it's recommended to use my name for ease of distinction). There is also a NEV_Price_System subdirectory, which contains the global configuration file for the Django framework (don't confuse it). The manage.py file is located in the root directory, while the setting.py file (Django global configuration) is in the NEV_Price_System subdirectory of the root directory. All commands that require the use of manage.py are run from the root directory. Finally, I uploaded a startup configuration file named lauch.json to the static directory. If you use VSCode, you can directly import it, so you don't have to enter commands in the terminal every time.   

In terms of data, the car_data_15features.xlsx file in static is generated by me using the original data (car_data.xlsx) to screen the top 15 most influential features based on feature importance in XGBoost. My writing scripts are located in car\management\commands, which include scripts for filling missing values, writing data, updating data, and training models. If you use your own data, you can find pd.read_excel in the writing and updating scripts, which is the directory where my files are written. You can modify it yourself, but try to ensure that the fields and features are consistent with my sample data, otherwise you will need to modify the written fields and types yourself.

The training model script module also has fixed parameters, and you can change the learning rate and early stopping conditions of XBGoost yourself.
# Future development path
In the future, I will design with the direction of facilitating collaborative development, focusing on enhancing the system's high adaptability to enable dynamic dataset writing, including online dynamic crawling (this part may take a longer time as it involves data permissions from some automotive company websites) and offline data writing (to prevent data leakage and contamination in the database, I will design developer roles to log in to my system and require approval under my administrator before data can be written). Secondly, I will optimize the prediction model by combining the original single XGBoost model with the MLP model into a larger model, and similar to ChatGPT's role positioning, I will focus the model more on car price prediction, as well as subsequent time series prediction. This is a difficult task, and my four years of university studies are still superficial. I hope for support from community experts.
